...
void IRCClient::Send_WoW_Channel(const char *channel, std::string chat)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        if (itr->second && itr->second->GetSession()->GetPlayer() && itr->second->GetSession()->GetPlayer()->IsInWorld())
        {
            ...
            {
                uint32 channelId;
                AreaTableEntry const* zone = sAreaTableStore.LookupEntry(itr->second->GetSession()->GetPlayer()->GetZoneId());
                if (cMgr->GetChannel(channelId, channel, itr->second->GetSession()->GetPlayer(), true, zone))
                {
                    WorldPacket data;
