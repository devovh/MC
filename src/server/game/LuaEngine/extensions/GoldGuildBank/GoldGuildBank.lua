...
Money.GoldMath = (math.floor(gold*0.0001)/10^0)
	...
	if(gold > 0) then
		if(player:IsInGuild()) then
			player:GetGuild():DepositBankMoney(player, (math.floor(gold*0.1)))
			player:SendBroadcastMessage("("..(math.floor(((gold))*convert)/10^0)..currency.." deposited to guild bank.)")
		end
	end
end
