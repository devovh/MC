...
void IRCCmd::Account_Player(_CDATA *CD)
{
    ...
    ObjectGuid guid = sCharacterCache->GetCharacterGuidByName(_PARAMS[0]);
    int account_id = 0;
    ...
    if (_PARAMS[1] == "ip")
    {
        ...
        if (ip != "")
        {
            ...
            if (Player* plr = GetPlayer(_PARAMS[0]))
                plr->GetSession()->KickPlayer(nullptr);
...
void IRCCmd::Char_Player(_CDATA *CD)
{
    ...
    if (plr)
    {
        ...
        {
            plr->UpdateWeaponsSkillsToMaxSkillsForLevel();
            ...
            if (skill > 0)
            {
                ...
                if (skilllookup)
                {
                    ...
                    if (plr->GetSkillValue(skill))
                    {
                        ...
                        Send_Player(plr, MakeMsg("Skill: %s Has Been Set To Level: %i Max: %i By: %s",skilllookup->DisplayName[0], level, max, CD->USER.c_str()));
                        Send_IRCA(ChanOrPM(CD), MakeMsg("\00313[%s] : Has Had Skill: %s Set To Level: %d Max: %d",_PARAMS[0].c_str() , skilllookup->DisplayName[0], level, max), true, CD->TYPE);
                    }
                    else
                        Send_IRCA(CD->USER, MakeMsg("Player Does Not Have The %s Skill Yet.", skilllookup->DisplayName[0]), true, "ERROR");
                }
                else
...
void IRCCmd::Info_Server(_CDATA *CD)
{
    ...
    char ircupdt [50];
    sprintf(ircupdt, "%u", sWorldUpdateTime.GetLastUpdateTime());
    ...
    Send_IRCA(ChanOrPM(CD), "\00310MotD: \xF"+(std::string)Motd::GetMotd(), true, CD->TYPE);
...
void IRCCmd::Item_Player(_CDATA *CD)
{
    ...
    if (_PARAMS[1] == "add")
    {
        ...
        Item* item = plTarget->StoreNewItem(dest, itemId, true, GenerateItemRandomPropertyId(itemId));
        if (count > 0 && item)
        {
...
void IRCCmd::Kill_Player(_CDATA *CD)
{
    ...
    if (Player* plr = GetPlayer(_PARAMS[0]))
    {
        if (plr->IsAlive())
        {
            Player::DealDamage(plr, plr, plr->GetHealth(), nullptr, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr, false);
            plr->SaveToDB();
            if (_PARAMS[1] == "")
                _PARAMS[1] = "No Reason Given.";
...
void IRCCmd::Player_Player(_CDATA *CD)
{
    ...
    if (sCharacterCache->GetCharacterGuidByName(_PARAMS[0].c_str()))
        plguid = sCharacterCache->GetCharacterGuidByName(_PARAMS[0].c_str());
    if (plguid > 0)
    {
        ...
        if (result)
        {
            ...
            if (ponline == "\x3\x30\x33Online")
            {
                ...
                if (plr)
                {
                    ...
                    ponline += " in " + (std::string) area->AreaName[sWorld->GetDefaultDbcLocale()];
                    if (area->ParentAreaID != 0)
                    {
                        AreaTableEntry const* zone = sAreaTableStore.LookupEntry(area->ParentAreaID);
                        ponline += " (" + (std::string)zone->AreaName[sWorld->GetDefaultDbcLocale()] + ")";
                    }
                }
            }
...
void IRCCmd::Lookup_Player(_CDATA *CD)
{
    ...
    if (_PARAMS[0] == "char")
    {
        ...
        if (sCharacterCache->GetCharacterGuidByName(_PARAMS[1].c_str()))
            plguid = sCharacterCache->GetCharacterGuidByName(_PARAMS[1].c_str());
        if (plguid > 0)
        {
    ...
    if (_PARAMS[0] == "skill")
    {
        ...
        if (atoi(skill.c_str()) > 0)
        {
            ...
            if (skillInfo)
            {
                std::string name = skillInfo->DisplayName[sWorld->GetDefaultDbcLocale()];
    ...
    if (_PARAMS[0] == "spell")
    {
        ...
        if (atoi(spell.c_str()) > 0)
        {
            ...
            if (spellInfo)
            {
                std::string name = spellInfo->Name[sWorld->GetDefaultDbcLocale()];
...
void IRCCmd::Level_Player(_CDATA *CD)
{
    ...
    ObjectGuid guid = sCharacterCache->GetCharacterGuidByName(player.c_str());
    ...
    } else
    {
        ...
        int32 i_oldlvl = chr ? chr->GetLevel() : sCharacterCache->GetCharacterLevelByGuid(level);
...
void IRCCmd::Mute_Player(_CDATA *CD)
{
    ...
    if (guid)
    {
        ...
        else
        {
            ...
            PreparedStatement<LoginDatabaseConnection>*  stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_MUTE_TIME);
...
void IRCCmd::Tele_Player(_CDATA *CD)
{
    ...
    else if (_PARAMS[1] == "to")
    {
        ...
        {
            if (ObjectGuid guid = sCharacterCache->GetCharacterGuidByName(_PARAMS[2].c_str()))
            {
                bool in_flight;
